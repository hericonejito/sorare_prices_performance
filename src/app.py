...
def update_figure(selected_player,gw_number):
    ...
    contender_df = df[np.logical_and(df['gw']==gw_number,df['player_name'].isin(contender_lineup))].groupby('player_name').max()[['team_name','against','tenGameAverage','score']].sort_values(by='score',ascending=False)
